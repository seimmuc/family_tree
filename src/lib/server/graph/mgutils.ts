...
export function toLocalDateTime<T extends Record<string, unknown>>(obj: T): ObjLDT<T> {
  return Object.fromEntries(
    Object.entries(obj).map(([k, v]) => [k, v instanceof Date ? LocalDateTime.fromStandardDate(v) : v])
  ) as ObjLDT<T>;
}

export function toJSDate<T extends Record<string, unknown>>(obj: T): ObjDate<T> {
  return Object.fromEntries(
    Object.entries(obj).map(([k, v]) => [k, v instanceof LocalDateTime ? v.toStandardDate() : v])
  ) as ObjDate<T>;
}
