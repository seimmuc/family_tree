...
export async function load() {
  // Warning: evil database writes on GET request lurk here
  const [peopleDeleted, peopleAdded] = await WriteActions.perform(async act => {
    const delCount = await act.deletePeople(
      filtersWhereAnd([{ prop: 'name', operator: 'IN', val: ALL_FIRST_NAMES }], 'p')
    );
    const pplCountStart = await act.countAllPeople();

    const hades = await act.addPerson({
      name: 'Hades',
      gender: 'male',
      bio: 'God of the dead and the king of the underworld'
    });
    ...
    const rhea = await act.addPerson({
      name: 'Rhea',
      gender: 'female',
      bio: 'Full-time mom and a part-time trickster'
    });
    ...
    const poseidon = await act.addPerson({
      name: 'Poseidon',
      gender: 'male',
      bio: 'Ruler of seas, winds, and even the earth(quakes). Also horses. Mostly just horses.'
    });
    await act.addPersonRelation(poseidon.id, rhea.id, 'PARENT');
    await act.addPersonRelation(poseidon.id, cronus.id, 'PARENT');

    const zeus = await act.addPerson({
      name: 'Zeus',
      gender: 'male',
      bio: 'God of thunder, aka the sky daddy. Sex god, but only figuratively.'
    });
    await act.addPersonRelation(zeus.id, rhea.id, 'PARENT');
    await act.addPersonRelation(zeus.id, cronus.id, 'PARENT');

    const persephone = await act.addPerson({
      name: 'Persephone',
      gender: 'female',
      bio: "I didn't choose the underworld, the king of the underworld chose me. Through kidnapping."
    });
    ...
    const aphrodite = await act.addPerson({
      name: 'Aphrodite',
      gender: 'female',
      bio: 'Goddess of beauty, love and lust'
    });
